...
```C
 ...
 arch/arm64/kernel/entry.S                          |    6 +	arm64: add seccomp support  secure computing
 ...
 arch/arm64/kernel/process.c                        |   66 +	arm64: process: dump memory around registers when displaying regs
							machine_shutdown中没有调用preempt_disable
...
第四章 sprdlinux3.10升级至3.18总结
----------------

* 总体评价: 3.18引入的主要新功能、新特性，对代码优化、功能增强、适应新ARM等技术指标有较大益处。
3.18同时拥有社区稳定分支和Google Android维护分支，适宜长期采用。预期升级风险可控。

* 对我司较为重要的更新

-- DT更加全面深入的支持
-- fbdev迁移至DRM
-- genpd: generic power domain框架实现
-- reservered memory: 通过DT静态和动态分配管理
-- syscon: 解决全局寄存器问题
-- psci已经支持0.2，我们如何跟进
-- 优化arm64 memcpy 对齐问题
-- dts文件中对cpu的描述需要考虑cluster/cpu拓扑结构
-- 打开CONFIG_NO_HZ_FULL_SYSIDLE是否降低功耗
-- 使用timefd代替alarm_dev，我们的关机闹钟驱动是否做出相应改动。
-- arm64工程defconfig增加相关处理废弃/过期指令处理选项
-- arm架构代码在poweroff/reboot时禁止抢占，arm64没有禁止，需要check
-- logd驱动改成pmsg方式，对log系统是否有影响
-- google已经增加cpufreq interactive驱动，我们跟进
